...
function Counter(){
    ...
    useEffect(
        ()=>{
            if(!isRunning) return;
            const interval=setInterval(()=>setCount(prev=>{
                if (prev==0){
    ...
    return(
        <>
            ...
            <button className="border-4 rounded-2xl border-red-800 font-bold text-[20px] mt-4 py-2 px-4" onClick={handleStart}>Start</button>
            ...
            <button className="border-4 rounded-2xl border-red-800 font-bold text-[20px] mt-4 py-2 px-4" onClick={handleStop}>Stop</button>
